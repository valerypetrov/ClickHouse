...
def test_move_takes_the_ingested_version_after_source_deleted(started_cluster):
    """A source deleted after the read has no current version, so the move looks up the version that
    was read. A re-upload of the same bytes right after that lookup must not be moved in its place."""
    node = started_cluster.instances["instance"]
    token = generate_random_string().lower()
    bucket = f"versioned-{token}"
    table_name = f"move_deleted_before_{token}"
    files_path = f"{table_name}_data"
    processed_prefix = f"{token}_moved"
    source_key = f"{files_path}/part.csv"
    destination_key = f"{processed_prefix}/part.csv"
    ...
    try:
        create_mv(node, table_name, f"{table_name}_dst")
        wait_failpoint_paused(node, PAUSE_BEFORE_POST_PROCESS_FAILPOINT)
        # The rows are read and inserted; a delete marker now hides the version that was read.
        client.delete_object(Bucket=bucket, Key=source_key)
        node.query(
            f"SYSTEM ENABLE FAILPOINT {PAUSE_AFTER_MOVE_SOURCE_LOOKUP_FAILPOINT}"
        )
    finally:
        node.query(f"SYSTEM DISABLE FAILPOINT {PAUSE_BEFORE_POST_PROCESS_FAILPOINT}")
    try:
        wait_failpoint_paused(node, PAUSE_AFTER_MOVE_SOURCE_LOOKUP_FAILPOINT)
        # The move has found the version that was read; the same bytes come back before the copy.
        reuploaded = client.put_object(
            Bucket=bucket, Key=source_key, Body=data, ContentType="text/plain"
        )["VersionId"]
        assert reuploaded != ingested
    ...
    wait_until(lambda: source_versions() == {reuploaded})
    head = client.head_object(Bucket=bucket, Key=destination_key)
    assert head["ContentType"] == "text/csv"
    assert head["Metadata"]["clickhouse_move_source_version_id"] == ingested
